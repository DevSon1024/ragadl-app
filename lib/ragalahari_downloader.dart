...
class _RagalahariDownloaderState extends State<RagalahariDownloader> with AutomaticKeepAliveClientMixin {
  ...
  void _initializeFields() {
    // Only initialize if not already initialized or if we have new data
    if (!_isInitialized ||
      ...
      // If we have all needed data and it's from a new navigation, auto-fetch images
      if (widget.initialUrl != null && widget.initialUrl!.isNotEmpty &&
          widget.initialFolder != null && widget.initialFolder!.isNotEmpty &&
          !_isInitialized) {
        // Use a microtask to ensure the UI is built first
        Future.microtask(() {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          automaticallyImplyLeading: Navigator.of(context).canPop(), // Only show back button if we can pop
          actions: [
            IconButton(
  ...
  // Store last back press time for double-back-to-exit functionality
